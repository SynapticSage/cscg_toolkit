"""
Train MNIST CNN with sensory-only CHMM (no action conditioning).
...
from models import MNISTWithCHMMSensory
...
def main():
    ...
    # Model (sensory-only, no actions)
    model = MNISTWithCHMMSensory(
        n_states=config['chmm']['n_states'],
        dropout=config['model']['dropout']
    ).to(device)

    print(f'Model: MNISTWithCHMMSensory (no actions)')
    ...
    writer = SummaryWriter(os.path.join(config['logging']['tensorboard_dir'], 'chmm_sensory'))
    ...
    for epoch in range(1, config['epochs'] + 1):
        ...
        if test_acc > best_acc:
            ...
            torch.save({
                ...
                'test_acc': test_acc,
            }, os.path.join(config['logging']['save_dir'], 'chmm_sensory_best.pth'))
